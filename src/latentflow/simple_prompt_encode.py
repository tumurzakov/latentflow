...
class SimplePromptEncode(Flow):
    def __init__(self,
        ...
        logging.debug("SimplePromptEncode init")
    ...
    def encode(self, prompt, negative_prompt, num_videos_per_prompt=1):
        ...
        logging.debug("SimplePromptEncode encode %s", text_input_ids)
        ...
        if self.do_classifier_free_guidance:
            ...
            if hasattr(self.text_encoder.config, "use_attention_mask") and self.text_encoder.config.use_attention_mask:
                attention_mask = uncond_input.attention_mask.to(self.text_encoder.device)
            else:
                attention_mask = None

            uncond_input_ids = uncond_input.input_ids
            uncond_embeddings = self.text_encoder(
                uncond_input_ids.to(self.text_encoder.device),
                attention_mask=attention_mask,
            )
