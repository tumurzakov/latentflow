...
class IPAdapterModelLoader:
    ...
    def load_ipadapter_model(self, ipadapter_file):
        if os.path.isfile(ipadapter_file):
            ckpt_path = ipadapter_file
        else:
...
class IPAdapterApply:
    ...
    def init(self, ipadapter, model, weight, clip_vision=None, image=None, weight_type="original", noise=None, embeds=None, attn_mask=None, start_at=0.0, end_at=1.0, unfold_batch=False, insightface=None, faceid_v2=False, weight_v2=False):
    ...
    def encode(self, ipadapter, model, weight, clip_vision=None, image=None, weight_type="original", noise=None, embeds=None, attn_mask=None, start_at=0.0, end_at=1.0, unfold_batch=False, insightface=None, faceid_v2=False, weight_v2=False):
        self.dtype = torch.float16 if comfy.model_management.should_use_fp16() else torch.float32
        self.device = comfy.model_management.get_torch_device()
        self.weight = weight
        self.is_full = "proj.3.weight" in ipadapter["image_proj"]
        ...
        if embeds is not None:
            ...
            if self.is_faceid:
                ...
                for i in range(face_img.shape[0]):
                    for size in [(size, size) for size in range(640, 128, -64)]:
                        ...
                        if face:
                            ...
                            if 640 not in size:
                                logging.debug(f"InsightFace detection resolution lowered to {size}")
                            break
                    else:
                ...
                if image.shape[1] != image.shape[2]:
                    logging.debug("the IPAdapter reference image is not a square, CLIPImageProcessor will resize and crop it at the center. If the main focus of the picture is not in the middle the result might not be what you are expecting.")
