...
video = \
    (Debug("Video to video")
        | VideoLoad(f'{data}/input/in.mp4', device='cuda', video_length=state['video_length'], start_frame=19, width=512, height=288)
        - VideoShow(fps=16)
        | VaeVideoEncode(
    ...
    (state['latent']
        ...
        | Loop(state['timesteps'], name="Denoise loop", callback=lambda timestep_index, timestep:
            (Latent(latent=torch.zeros_like(state['latent'].latent).repeat(2,1,1,1,1)) > state('noise_predict')) >> \
            (state
                 | Loop(
                    TileGenerator(
                        Tile(
                    ...
                    callback=lambda tile_index, tile:
                        ...
                        state['tile_latent'] 
                            ...
                            > NoisePredict(state['noise_predict'].latent[tile])
